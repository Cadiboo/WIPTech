...
public class ModEnums {
	...
	BAUXITE(18, new ModMaterialProperties(true, false, false, false, false,
		ModMaterials.ALUMINIUM.getProperties().getHardness(), 0));
...
//		private ItemModArmor	boots;
	...
	ModMaterials(final int idIn, final ModMaterialProperties propertiesIn) {
		...
		String textureName = new ResourceLocation(
			this.getResouceLocationDomain("helmet", ForgeRegistries.ITEMS), getNameLowercase()).toString();

		int durability = Math
			.round(this.getProperties().getHardness() * ModReference.ARMOR_MATERIAL_HARDNESS_MULTIPLIER);
		...
		this.armorMaterial = EnumHelper.addArmorMaterial(name, textureName, durability, reductionAmounts,
			enchantability, soundOnEquip, toughness);
		this.armorMaterial.setRepairItem(new ItemStack(ForgeRegistries.ITEMS.getValue(new ResourceLocation(
			this.getResouceLocationDomain("helmet", ForgeRegistries.ITEMS), getNameLowercase()))));
	    } else
		...
		this.toolMaterial = EnumHelper.addToolMaterial(name, harvestLevel, maxUses, efficiency, damageVsEntity,
			enchantability);
	    } else
	...
	public final String getResouceLocationDomain(String nameSuffix, IForgeRegistry registry) {
		...
		if (mod.getModId() != ModReference.ID)
		    if (registry.containsKey(new ResourceLocation(mod.getModId(),
			    getVanillaNameLowercase(nameSuffix) + "_" + nameSuffix)))
			return mod.getModId();
	    }
	...
	@Nullable
	private <T> T getRegistryValue(@Nonnull IForgeRegistry<? extends IForgeRegistryEntry<T>> registry,
		@Nonnull String nameSuffix) {
	    nameSuffix = nameSuffix.toLowerCase();
	    return (T) registry.getValue(new ResourceLocation(this.getResouceLocationDomain(nameSuffix, registry),
		    this.getNameLowercase() + "_" + nameSuffix));
	}
