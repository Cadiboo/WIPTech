...
public final class EventSubscriber {
	...
	GameRegistry.registerTileEntity(clazz,
		new ResourceLocation(ModReference.ID, getRegistryNameForClass(clazz, "TileEntity")));
    }
	...
	for (ModMaterials material : ModMaterials.values()) {
	    if (material.getProperties().hasOre())
		registry.register(new ModItemBlock(material.getOre(),
			new ResourceLocation(material.getResouceLocationDomain("ore", ForgeRegistries.ITEMS),
				material.getNameLowercase() + "_ore")));

	    if (material.getProperties().hasBlock())
		registry.register(new ModItemBlock(material.getBlock(),
			new ResourceLocation(material.getResouceLocationDomain("block", ForgeRegistries.ITEMS),
				material.getNameLowercase() + "_block")));

	    if (material.getProperties().hasIngotAndNugget()) {
		registry.register(new ModItemBlock(material.getIngot(),
			new ResourceLocation(material.getResouceLocationDomain("ingot", ForgeRegistries.ITEMS),
				material.getNameLowercase() + "_ingot")));
		registry.register(new ModItemBlock(material.getNugget(),
			new ResourceLocation(material.getResouceLocationDomain("nugget", ForgeRegistries.ITEMS),
				material.getNameLowercase() + "_nugget")));
	    }
...
//	event.getRegistry().register(buildEntityEntryFromClass(EntityNapalm.class, false, true));
    ...
    private static final EntityEntry buildEntityEntryFromClass(final Class<? extends ModEntity> clazz,
	    final boolean hasEgg, final boolean sendVelocityUpdates) {

	return buildEntityEntryFromClassWithName(clazz,
		new ResourceLocation(ModReference.ID, getRegistryNameForClass(clazz, "Entity")), hasEgg,
		sendVelocityUpdates);

    }

    private static final EntityEntry buildEntityEntryFromClassWithName(final Class<? extends ModEntity> clazz,
	    final ResourceLocation registryName, final boolean hasEgg, final boolean sendVelocityUpdates) {
	EntityEntryBuilder<Entity> builder = EntityEntryBuilder.create();
	builder = builder.entity(clazz);
	builder = builder.id(registryName, entityId++);
	builder = builder.name(registryName.getResourcePath());
	...
	return org.apache.commons.lang3.StringUtils.uncapitalize(clazz.getSimpleName().replace(removeType, ""))
	...
	RenderingRegistry.registerEntityRenderingHandler(EntityPortableGenerator.class,
		renderManager -> new EntityPortableGeneratorRenderer(renderManager));
	RenderingRegistry.registerEntityRenderingHandler(EntityRailgunCasing.class,
		renderManager -> new EntityRailgunCasingRenderer(renderManager));
	RenderingRegistry.registerEntityRenderingHandler(EntityRailgunSlug.class,
	...
	for (ModMaterials material : ModMaterials.values()) {
		...
		ModelLoader.setCustomStateMapper(material.getWire(), new StateMapperBase() {
			...
			return new ModelResourceLocation(
				new ResourceLocation(ModReference.ID, material.getNameLowercase() + "_wire"),
		...
		ModelLoader.setCustomStateMapper(material.getEnamel(), new StateMapperBase() {
		    ...
		    protected ModelResourceLocation getModelResourceLocation(IBlockState iBlockState) {
			return new ModelResourceLocation(
				new ResourceLocation(ModReference.ID, material.getNameLowercase() + "_enamel"),
				ModWritingUtil.default_variant_name);
		    }
		});
	    }
	...
	ModMaterials material : ModMaterials.values()) {
		...
		if (material.getIngot() != null
			&& material.getResouceLocationDomain("ingot", ForgeRegistries.ITEMS).equals(ModReference.ID))
		    registerItemBlockModel(material.getIngot());
		if (material.getNugget() != null
			&& material.getResouceLocationDomain("nugget", ForgeRegistries.ITEMS).equals(ModReference.ID))
	...
	ModelLoader.setCustomModelResourceLocation(item, 0,
		new ModelResourceLocation(item.getRegistryName(), ModWritingUtil.default_variant_name));
    }
	...
	ModelLoader.setCustomModelResourceLocation(
		ForgeRegistries.ITEMS
			.getValue(new ResourceLocation("minecraft", block.getRegistryName().getResourcePath())),
		0, new ModelResourceLocation(block.getRegistryName(), ModWritingUtil.default_variant_name));
    }
...
//				Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation(ModReference.ID, "textures/gui/turbine.png"));
		...
		drawNonStandardTexturedRect(Width + 1, Height + 1 + scaled_height, 176, 0, 8, 52 - scaled_height, 256,
			256);
		int percent = (int) Math.round(power * 100);
		mc.fontRenderer.drawStringWithShadow(percent + "%", Width - 7 - String.valueOf(percent).length() * 6,
			Height + 35, 0xFFFFFF);
    ...
    private static final void drawNonStandardTexturedRect(int x, int y, int u, int v, int width, int height,
	...
	if (item instanceof ItemCoil)
	    setTooltip(event, WIPTech.proxy.localize("conductivity") + ": "
		    + ((ItemCoil) item).getModMaterial().getProperties().getConductivity() + "%");

	if (item instanceof ItemRail)
	    setTooltip(event, WIPTech.proxy.localize("conductivity") + ": "
		    + ((ItemRail) item).getModMaterial().getProperties().getConductivity() + "%");

	if (Block.getBlockFromItem(item) instanceof BlockWire
		&& !(Block.getBlockFromItem(item) instanceof BlockEnamel)) {
	    setTooltip(event, WIPTech.proxy.localize("conductivity") + ": "
		    + ((BlockWire) Block.getBlockFromItem(item)).getModMaterial().getProperties().getConductivity()
		    + "%");
	...
	if (Block.getBlockFromItem(item) instanceof BlockEnamel)
	    setTooltip(event, WIPTech.proxy.localize("conductivity") + ": "
		    + ((BlockEnamel) Block.getBlockFromItem(item)).getModMaterial().getProperties().getConductivity()
		    + "%");
	...
	for (int i = 0; i < event.getToolTip().size(); i++) {
	    if (net.minecraft.util.StringUtils.stripControlCodes(event.getToolTip().get(i))
		    .equals(event.getItemStack().getItem().getRegistryName().toString())) { // TODO why and what does
											    // this do???
