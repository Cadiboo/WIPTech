	...
	public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable NBTTagCompound nbt) {
		return new ICapabilityProvider() {
			...
			public <T> T getCapability(@Nonnull Capability<T> capability, @Nullable EnumFacing facing) {
				...
				return capability == CapabilityEnergy.ENERGY
						? CapabilityEnergy.ENERGY.cast(new EnergyStorage(maxEnergy, maxTransfer, maxTransfer,
								stack.getTagCompound() != null ? stack.getTagCompound().getInteger("Energy") : 0) {
							@Override
							...
							private void setEnergy(int energy) {
								NBTTagCompound nbt = stack.getTagCompound() != null ? stack.getTagCompound()
										: new NBTTagCompound();
					...
					if (energy != null && energy.canExtract()) {
						...
						if (storage != null && storage.canReceive()) {
							storage.receiveEnergy(
									energy.extractEnergy(storage.receiveEnergy(Integer.MAX_VALUE, true), false), false);
						}
					}
				}
