package cadiboo.wiptech.block.state;
...
public class BlockStrongPistonStructureHelper extends BlockPistonStructureHelper {
	...
	/**
	 * This is a List<BlockPos> of blocks that will be destroyed when a piston
	...
	public boolean canMove() {
		...
		if (!BlockStrongPistonBase.canPush(iblockstate, this.world, this.blockToMove, this.moveDirection, false, this.moveDirection)) {
			...
			for (int i = 0; i < this.toMove.size(); ++i) {
				...
				if (this.world.getBlockState(blockpos).getBlock().isStickyBlock(this.world.getBlockState(blockpos))
	...
	private boolean addBlockLine(BlockPos origin, EnumFacing p_177251_2_) {
		...
		if (iblockstate.getBlock().isAir(iblockstate, this.world, origin)) {
			return true;
		} else if (!BlockStrongPistonBase.canPush(iblockstate, this.world, origin, this.moveDirection, false, p_177251_2_)) {
			return true;
			...
			if (i + this.toMove.size() > 12) {
				...
				while (block.isStickyBlock(iblockstate)) {
					...
					if (iblockstate.getBlock().isAir(iblockstate, this.world, blockpos) || !BlockStrongPistonBase.canPush(iblockstate, this.world,
							blockpos, this.moveDirection, false, this.moveDirection.getOpposite()) || blockpos.equals(this.pistonPos)) {
						break;
					}
				...
				while (true) {
					...
					if (k > -1) {
						...
						for (int l = 0; l <= k + i1; ++l) {
							...
							if (this.world.getBlockState(blockpos2).getBlock().isStickyBlock(this.world.getBlockState(blockpos2))
					...
					if (!BlockStrongPistonBase.canPush(iblockstate, this.world, blockpos1, this.moveDirection, true, this.moveDirection)
	...
	/**
	 * Returns a List<BlockPos> of all the blocks that are being moved by the
	 * piston.
	 */
	@Override
	...
	/**
	 * Returns an List<BlockPos> of all the blocks that are being destroyed by the
	 * piston.
	 */
	@Override
