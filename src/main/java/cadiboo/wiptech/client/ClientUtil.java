...
public class ClientUtil {
	...
	GlStateManager.rotate(
		face == EnumFacing.DOWN ? 0
			: face == EnumFacing.UP ? 180F
				: face == EnumFacing.NORTH || face == EnumFacing.EAST ? 90F : -90F,
...
//	OpenGlHelper.setLightmapTextureCoords(int target, float p_77475_1_, float t)
     ...
     * @author Cadiboo
     */
    public static final void renderLightning(final int number, final int NumberOfBranches,
	    final int NumberOfPossibleSubBranches, final double scale) {
	...
	for (int shells = 0; shells < 4; ++shells) {
		...
		for (int possibleSubBranches = 0; possibleSubBranches < NumberOfPossibleSubBranches
			+ 1; ++possibleSubBranches) {
		    int position = 7;
			...
			for (int side = 0; side < 5; ++side) {
			    ...
			    bufferbuilder
				    .pos(bottomOffsetX + topTranslateX * scale16, yPos * scale,
					    bottomOffsetZ + topTranslateZ * scale16)
				    .color(0.45F, 0.45F, 0.5F, 0.3F).endVertex();
			    bufferbuilder
				    .pos(topOffsetX + bottomTranslateX * scale16, (yPos + 1) * scale,
					    topOffsetZ + bottomTranslateZ * scale16)
				    .color(0.45F, 0.45F, 0.5F, 0.3F).endVertex();
			}
	...
	double yGround = entity.motionY + yAdd == 0 && entity.prevPosY > entity.posY ? entity.posY - entity.prevPosY
		: 0;
	double xFall = 1;
	...
	double dX = entity.posX - (entity.prevPosX - entity.posX) * partialTicks
		- (entity.motionX * xFall) * flyingMultiplier;
	double dY = entity.posY - yGround - (entity.prevPosY - entity.posY) * partialTicks
		- (entity.motionY + yAdd) * yFlying;
	double dZ = entity.posZ - (entity.prevPosZ - entity.posZ) * partialTicks
		- (entity.motionZ * zFall) * flyingMultiplier;
	...
	double _Angle = Math.atan2(destination.y,
		Math.sqrt(destination.x * destination.x + destination.z * destination.z));
...
//						if (possibleSubBranches == 0) { // Main branch
     ...
     * @author Cadiboo
     */
    public static final void drawCuboid(float minU, float maxU, float minV, float maxV, double x_size, double y_size,
	    double z_size, double scale) {
     ...
     * @author Cadiboo
     */
    public static final void drawSeamlessCuboid(float minU, float maxU, float minV, float maxV, double z_size,
	    double y_size, double x_size, double scale) {
     ...
     * @author Cadiboo
     */
    public static final void drawQuad(float minU, float maxU, float minV, float maxV, double x_size, double y_size,
	    double scale) {
	GlStateManager.scale(scale, scale, scale);
