...
public class EntityPortableGeneratorRenderer extends Render<EntityPortableGenerator> {
	...
	public void doRender(final EntityPortableGenerator entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
		...
		try {
			...
			try {
				GlStateManager.pushMatrix();
				ClientUtil.renderModel(ModelsCache.INSTANCE.getBakedModel(new ModResourceLocation(ModReference.ID, "entity/portable_generator_body")));
			...
			try {
				...
				ClientUtil.renderModel(ModelsCache.INSTANCE.getBakedModel(new ModResourceLocation(ModReference.ID, "entity/portable_generator_handle")));
				GlStateManager.popMatrix();
			} catch (Exception e) {
			...
			try {
				for (int wheelIndex = 0; wheelIndex < 4; wheelIndex++) {
					...
					ClientUtil.renderModel(ModelsCache.INSTANCE.getBakedModel(new ModResourceLocation(ModReference.ID, "entity/portable_generator_wheel")));
					GlStateManager.popMatrix();
				}
			} catch (Exception e) {
