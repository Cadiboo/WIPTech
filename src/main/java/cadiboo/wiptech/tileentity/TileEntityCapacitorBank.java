...
public class TileEntityCapacitorBank extends TileEntityBase implements ITickable {
	...
	public static final int getSlots() {
		return 8;
	}
	...
	/*
	 * @Override public void onLoad() { Block block =
	 * world.getBlockState(this.getPos()).getBlock(); if(block instanceof
	 * BlockCapacitorBank){ int oldEnergy = this.energy.getEnergyStored(); //
	...
	/*
	 * public int getChargingState(){ IEnergyStorage storage = this.normal;
	 * if(storage != null && storage.getMaxEnergyStored() > 0){ return (int)
	 * (storage.getEnergyStored() / (storage.getMaxEnergyStored() * 1.0F) * 10.0F);
	 * } return 0; }
	 * 
	 * public EnumFacing getInputSide(){ return this.world != null ?
	 * this.world.getBlockState(this.pos).getValue(BlockDirectional.FACING) :
	 * EnumFacing.NORTH; }
	 * 
	 * public EnumFacing getOutputSide(){ return this.getInputSide().getOpposite();
	 * }
	 */
